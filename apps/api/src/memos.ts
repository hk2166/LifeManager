...
export function registerMemoRoutes(app: express.Express) {
  ...
  app.post('/api/memos', upload.single('audio'), h(async (req, res) => {
    if (!req.file) return res.status(400).json({ error: 'audio file missing' });
    const { rows: ins } = await q(
      `INSERT INTO memos (audio_path, status) VALUES ($1, 'processing') RETURNING id`,
      [req.file.path]
    );
  ...
  app.post('/api/memos/:id/confirm', h(async (req, res) => {
    const type = String(req.body?.type ?? '');
    const { rows } = await q('SELECT * FROM memos WHERE id = $1', [req.params.id]);
  ...
  app.get('/api/memos/:id/audio', h(async (req, res) => {
    const { rows } = await q('SELECT audio_path FROM memos WHERE id = $1', [req.params.id]);
