...
export async function anticipate() {
  ...
  // rerun-safe: this pass owns email-sourced reminders, so replace them wholesale
  ...
  for (const it of Array.isArray(out.items) ? out.items : []) {
    ...
    await q(
      `INSERT INTO items (type, title, due_at, confidence, source_kind, source_message_id)
       VALUES ('reminder', $1, $2, $3, 'email', $4)`,
      [it.title, due, it.confidence, it.source_message_id]
    );
    n++;
  }
